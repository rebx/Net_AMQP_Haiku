...
our @EXPORT = qw(DEFAULT_HOST DEFAULT_PROTO DEFAULT_PORT DEFAULT_VHOST
    ...
    DEFAULT_USERNAME DEFAULT_PASSWORD FLAG_DEBUG
...
use constant DEFAULT_EXCHANGE => '';    # the name of the exchange you want to use
use constant DEFAULT_PUBLISH_WEIGHT => 0;    # frame weight
use constant FLAG_MANDATORY         => 0;    # mandatory routing
use constant FLAG_IMMEDIATE         => 0; # for flagging immediate delivery. if set to 1, it will guarantee that the message can be consumed.
use constant FLAG_DELIVERY => 1;    # for flagging delivery
use constant FLAG_PRIORITY => 1;    # Basic::ContentHeader priority flag
use constant FLAG_PASSIVE  => 0; # Exchange::Declare passive flag. If set, it will instruct the server not to create the exchange
use constant FLAG_NO_ACK    => 1;    # frame ack
use constant FLAG_EXCLUSIVE => 0;    # request exclusive consumer access
use constant FLAG_DURABLE => 0;    # set queue as durable. Let the mnesia db bring it back on reboot
